// NOTE: This isn't actually used by ./helper.rb, but could be used for an
//  alternate implementation where the #capybara_lockstep_js override uses this
//  file directly, which would actually make it possible for
//  app/assets/javascripts/vendor/capybara-lockstep.js to go away since the
//  injected script would have everything needed to work with Turbolinks.
//
// noinspection LongLine, LocalVariableNamingConventionJS, FunctionWithInconsistentReturnsJS, UnnecessaryReturnStatementJS, JSUnusedLocalSymbols, OverlyComplexBooleanExpressionJS, JSUnusedGlobalSymbols, JSFileReferences
...
window.CapybaraLockstep = (function() {
  ...
  function trackJQuery() {
    ...
    whenReady(function() {
      ...
      // async using setTimeout()). Hence we need to wait for it in addition to XHR.
      //
...
//$(document).on('turbolinks:load', () => CapybaraLockstep.track());
